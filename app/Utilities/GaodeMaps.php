...
class GaodeMaps
{
    ...
    public  static function geocodeAddress($address, $city, $state)
    {
        ...
        $client = new Client();
        //  get  构建地理编码 API 请求 URL，默认返回 JSON 格式响应
        $response = $client->request('GET', 'https://restapi.amap.com/v3/geocode/geo?address=' . $address . '&key=' . $apiKey);

        // 发送请求并获取响应数据
        $geocodeResponse = $response->getBody();
